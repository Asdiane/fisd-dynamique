...
import { Contact } from '../../../core/content.models';
...
export class AdminContactsComponent implements OnInit {
  ...
  async deleteContact(contact: Contact): Promise<void> {
    ...
    this.contactsService.delete(contact.id).subscribe({
      next: () => {
        this.snackbar.success(this.translate.instant('AdminContacts.DeleteSuccess'));
        this.refresh();
      },
      error: (err) => this.snackbar.error(err?.error?.error ?? this.translate.instant('AdminContacts.DeleteFailed'))
    });
  }
}
